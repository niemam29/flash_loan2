...
describe('Liquidity Pool tests - reward token with 18 decimals', function () {
    before(async function () {
        ...
        await TokenA.transfer(user.address, '1000000000000000000')
        await TokenA.connect(user).approve(Pool.address, '1000000000000000000')
    })
    it('Should deposit one token', async function () {
        await Pool.connect(user).deposit('1000000000000000000')
    ...
    it('Should withdraw all of user deposit', async function () {
        ...
        await Pool.connect(user).withdraw('999999999999999999')
        expect(await LpToken.balanceOf(user.address)).to.be.eq(0)
        expect(await TokenA.balanceOf(user.address)).to.be.eq('1000000000000000000')
    })
})
